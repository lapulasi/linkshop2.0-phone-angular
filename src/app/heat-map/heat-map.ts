...
export class HeatMap {
  ...
  originalWidth = 640; //  1280 640
  originalHeight = 480; //  720  480
  ...
  transformCoordinate(position: any): any {
    ...
    // return { x: (Math.floor(posX * currWidth / this.originalWidth / 4)  ) * 4 + 2,
    //   y: (Math.floor(posY * currHeight / this.originalHeight / 4) ) * 4 + 2,
    //   value: 1};
    //
    return {x: Math.floor(position.x * currWidth / 1280 ),
    y: Math.floor(position.y * currHeight / 720),
